...
const obtenerProductosEliminados = async (req, res) => {
    try {
        const results = await productSchema.findWithDeleted({ deleted: true }).populate("categories");

        console.log("Productos eliminados encontrados:", results.length);

        res.status(200).json({ data: results.map(productDTO) });
    } catch (error) {
        res.status(500).json({ mensaje: "Internal Server Error" });
    }
};
