...
export const Route = createFileRoute("/admin/ordenes/$orderId")({
  component: OrderDetail,
});

function OrderDetail() {
  const { orderId } = useParams({ from: "/admin/ordenes/$orderId" });
  ...
  const loadOrder = async () => {
    ...
    try {
      ...
      setOrder(data);
      const userData = await Api.fetchUserProfile(data.userId);
      setUser(userData);
    } catch {
  ...
  if (error || !order) {
    return (
      <div className="empty-state">
        ...
        <Link to="/admin/ordenes" className="auth-btn-primary" style={{ display: "inline-block", marginTop: "1.5rem", padding: "0.75rem 2rem", textDecoration: "none" }}>
          Volver a listado de órdenes
        </Link>
      </div>
    );
  }
  ...
  return (
    <>
      <Link to="/admin/ordenes" className="auth-link" style={{ display: "inline-flex", alignItems: "center", gap: "0.25rem", marginBottom: "1.5rem", fontSize: "0.9rem" }}>
        <ChevronLeft size={16} /> Volver a listado de órdenes
      </Link>
